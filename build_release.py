...
def archives():
    ...
    for name, files in packages.items():
        ...
        with ZipFile(output / name) as archive:
            ...
            assert 'progressi_sequenza.json' not in archive.namelist()
...
if __name__ == '__main__':
    ...
    subprocess.run([sys.executable, '-m', 'PyInstaller', '--noconfirm', '--distpath', '.', 'OfficinaDati.spec'], check=True)
    report = ROOT / 'build' / 'smoke-windows.json'
    report.unlink(missing_ok=True)
    subprocess.run([str(ROOT / 'OfficinaDati.exe'), '--smoke-test', str(report)], check=True, timeout=120,
                   creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
    assert json.loads(report.read_text(encoding='utf-8'))['ok'] is True
    archives()
    print('Pronti: OfficinaDati.exe e i due ZIP in dist.')
